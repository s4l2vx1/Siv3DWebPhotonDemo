...
#	pragma warning(disable:4355)
	...
	, mLoadBalancingClient(*this, appID, appVersion, ExitGames::LoadBalancing::ClientConstructOptions().setAutoLobbyStats(autoLobbyStats).setRegionSelectionMode(regionSelectionMode))
	, mLastInput(INPUT_NON)
#if defined _EG_EMSCRIPTEN_PLATFORM || defined _EG_XB1_PLATFORM || defined _EG_SWITCH_PLATFORM || defined _EG_GAMECORE_PLATFORM || defined _EG_PS5_PLATFORM
	, mAutoJoinRoom(true)
#else
	, mAutoJoinRoom(false)
#endif
...
void NetworkLogic::connect(void)
{
	...
	authValues.setType(ExitGames::LoadBalancing::CustomAuthenticationType::NINTENDO_SWITCH).setParameters(ExitGames::Common::JString(L"appversion=1") + L"&token=" + retrieveIDToken());
#elif defined _EG_SONY_PLATFORM
		...
		switch(state)
		{
			...
			case STATE_CONNECTED:
				switch(mLastInput)
				{
					...
					case INPUT_3:
						mpOutputListener->writeLine(L"\n============= Create Game Direct All To Others");
						opCreateRoom(ExitGames::LoadBalancing::DirectMode::ALL_TO_OTHERS);
						break;
					case INPUT_4:
						mpOutputListener->writeLine(L"\n============= Create Game Direct Master To Others");
						opCreateRoom(ExitGames::LoadBalancing::DirectMode::MASTER_TO_OTHERS);
						break;
#endif
...
void NetworkLogic::sendDirect(int64 count)
{
	...
	case 0: // send to all with one call
		{
			int sent = mLoadBalancingClient.sendDirect(str, ExitGames::LoadBalancing::SendDirectOptions().setFallbackRelay(fallbackRelay));
	...
	case 1: // send to a few with one call
		{
			...
			int sent = mLoadBalancingClient.sendDirect(str, ExitGames::LoadBalancing::SendDirectOptions().setTargetPlayers(targets).setFallbackRelay(fallbackRelay));
	...
	case 2: // send with call per player			
		{
			for(int i=0; i<mLoadBalancingClient.getCurrentlyJoinedRoom().getPlayerCount(); ++i) 
			{
				...
				if(nr != localNr)
				{
					if(mLoadBalancingClient.sendDirect(str, ExitGames::LoadBalancing::SendDirectOptions().setTargetPlayers(ExitGames::Common::JVector<int>(&nr, 1)).setFallbackRelay(fallbackRelay)))
...
void NetworkLogic::onAvailableRegions(const ExitGames::Common::JVector<ExitGames::Common::JString>& availableRegions, const ExitGames::Common::JVector<ExitGames::Common::JString>& availableRegionServers)
{
	...
	// select region "eu", if available, fall back to the first region from the list otherwise
